<script setup>
import { LifeBuoy, FileText, HelpCircle, ArrowUp } from 'lucide-vue-next'
import { Skeleton } from '~/components/ui/skeleton'

const api = useApi()
const { $i18n } = useNuxtApp()

const { data: support, pending, error } = await useAsyncData('support-page', async () => {
...
const content = computed(() => {
  ...
  return support.value?.[locale] || support.value?.ar || support.value?.en || ''
})
...
<template>
  <div class="sp-page">
    ...
    <!-- ── Document Header ── -->
    <header class="sp-doc-head">
      <div class="sp-doc-head-inner">
        <div class="sp-eyebrow">
          <LifeBuoy :size="13" />
          <span>{{ $t('support_subtitle') }}</span>
        </div>
        <h1 class="sp-doc-title">{{ $t('support') }}</h1>
        <div class="sp-doc-meta">
          <span class="sp-doc-meta-pill">
            <FileText :size="12" />
    ...
    <main class="sp-main">
      <article class="sp-doc">
        <div v-if="pending" class="sp-skeletons">
        ...
        <div v-else-if="error || !content" class="sp-empty">
          <HelpCircle :size="48" class="text-secondary opacity-40" />
          <p>{{ $t('no_result') }}</p>
        </div>
        ...
        <footer v-if="!pending && content" class="sp-doc-foot">
          <div class="sp-doc-foot-line"></div>
          <p class="sp-doc-foot-text">
            <LifeBuoy :size="14" />
            <span>{{ $t('support') }} · {{ updatedLabel }}</span>
          </p>
        </footer>
      </article>
    </main>
...
export default {
  name: 'Support',
  head() {
    return this.metaBuilder(this.$t('support'));
  },
};
</script>
...
<style>
/* ════════════════════════════════════════
   Support — editorial document layout
   ════════════════════════════════════════ */
.sp-page {
...
.sp-prose[dir="rtl"],
.sp-prose[dir="rtl"] *:not(code):not(pre) {
  text-align: right !important;
  direction: rtl !important;
}
.sp-prose[dir="ltr"],
.sp-prose[dir="ltr"] *:not(code):not(pre) {
  text-align: left !important;
  direction: ltr !important;
}
.sp-prose .text-center,
.sp-prose [style*="text-align: center"] {
